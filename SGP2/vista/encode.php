...
$ssql = "select * from tbcliente where id_cliente like '%" . $datoBuscar . "%'";
$rs = mysql_query($ssql);
...
while ($fila = mysql_fetch_array($rs)){
   array_push($arrayElementos, new ElementoAutocompletar(utf8_encode($fila["nombre_cliente"].' '.$fila['apellido']), $fila["id_cliente"]));
}

 ...
 *
 *	Autor		:		Marco A. Madue�o Mej�a <mmaduenophperu.net>
 *
 *	Version		:		2.1
 *
...
class PHPPaging {
	...
	**/
	function nombreVariable ($var) {
		...
		if(!preg_match("/(^[a-zA-Z0-9]+)$/i",$var)) return $this->error(true, "El nombre de la variable indicado contiene caracteres no v�lidos");
	...
	function error($desarrollo = true, $msg, $query = false, $limit_error = null) {
		if($this->modo != 'publicacion' and $this->mostre_error == false) {
			if($this->modo == 'reporte') {
				...
				$error.= "Si no logra solucionar el problema, env�e un mensaje a <code>phppagingphperu.net</code> indicando el error mostrado y la consulta que gener� el error";
			} else {
				$error = "Hubo un error ejecutando la paginaci�n. El mensaje devuelto es: <blockquote><code style='color: #080; font-size: 13px;'><strong>$msg</strong></code></blockquote>Si no logra solucionar el problema, env�e un mensaje a <code>phppagingphperu.net</code> indicando el error mostrado y la consulta que gener� el error";
			}
